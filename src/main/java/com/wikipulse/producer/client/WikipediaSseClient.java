...
public class WikipediaSseClient {
    ...
    private static final String WIKIPEDIA_STREAM_URL = "https://stream.wikimedia.org/v2/stream/recentchange";
    ...
    public void consumeStream() {
        ...
        webClient.get()
                ...
                .doOnNext(event -> {
                    ...
                    if (count % 20 == 0) {
                        log.info("Parsed event #{}: User: '{}', Title: '{}'", count, event.user(), event.title());
                    }
                })
                .doOnError(e -> log.error("Stream error: {}", e.getMessage()))
                .retryWhen(Retry.backoff(Long.MAX_VALUE, Duration.ofSeconds(2))
                        .maxBackoff(Duration.ofSeconds(30))
                        .doBeforeRetry(retrySignal -> log.warn("Connection dropped. Reconnecting... (Attempt #{})", retrySignal.totalRetries() + 1)))
                .subscribe(
                        event -> {},
    ...
    private WikiEditEvent mapToWikiEditEvent(WikipediaSseEvent sseEvent) {
        ...
        if (idObj instanceof Number num) {
            ...
            try { id = Long.parseLong(str); } catch (Exception ignored) {}
        }
